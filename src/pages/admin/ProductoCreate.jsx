  ...
  const handleSubmit = async (values) => {
    ...
    try {
       ...
       if (error.response && error.response.data.message.includes('ya está en uso')) {
        ...
        toast.error(error.response.data.message); // También muestra un mensaje de error con Toast
      } else {
        /* setError('Hubo un error al crear el producto'); */
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="p-6 bg-white rounded-lg shadow-md w-full max-w-lg">
        <ToastContainer />
        <h2 className="text-xl md:text-2xl font-semibold mb-4 text-center">Nuevo Producto</h2>
